...
export class BrowserAgent {
  ...
  async typeText(text){this.requirePage();await this.page.keyboard.insertText(String(text));return{ok:true,action:'typeText',length:String(text).length,url:this.page.url()};}
  async clickAt(x,y){this.requirePage();const px=Number(x),py=Number(y);if(!Number.isFinite(px)||!Number.isFinite(py))throw new Error('clickAt requires numeric args.x and args.y');await this.page.mouse.click(px,py);return{ok:true,action:'clickAt',x:px,y:py,url:this.page.url()};}
  async clickText(text,exact=true){this.requirePage();const value=String(text);await this.page.getByText(value,{exact:Boolean(exact)}).filter({visible:true}).first().click({timeout:10000});return{ok:true,action:'clickText',text:value,exact:Boolean(exact),url:this.page.url()};}
