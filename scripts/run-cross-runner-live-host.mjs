...
const hostMinutes=Math.min(350,Math.max(1,Number(process.env.CROSS_RUNNER_HOST_MINUTES||20)));
...
try {
  ...
  const chatgptResult=challenge?'challenge':promptVisible>0?'chatgpt-ui':'page-loaded';
  ...
  console.log('CHATGPT_CONNECTION_RESULT navigation-error');
...
console.log(`CROSS_RUNNER_HOST_WINDOW_MINUTES ${hostMinutes}`);
try{
 const deadline=Date.now()+hostMinutes*60_000;
 while(Date.now()<deadline){
  ...
  if(rows?.[0]?.state==='error') console.log(`CROSS_RUNNER_AGENT_ERROR_IGNORED ${JSON.stringify(rows[0].last_error||'Agent reported error')}`);
