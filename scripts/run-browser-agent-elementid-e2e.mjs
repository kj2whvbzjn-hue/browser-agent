...
const html = `<!doctype html><html><head><meta charset="utf-8"><title>Browser Agent elementId E2E</title></head><body>
...
<script>
  document.querySelector('#submit').addEventListener('click', () => {
    document.querySelector('#result').textContent = 'RESULT:' + document.querySelector('#message').value;
  });
</script>
</body></html>`;
...
try {
  ...
  const textbox = state1.elements.find((e) => e.role === 'textbox' && e.editable && e.label === 'Message');
  ...
  await fs.writeFile(path.join(outputDir, 'result.json'), JSON.stringify({ ok: true, finalState }, null, 2));
