...
const bearerToken = process.env.BROWSER_CONNECTOR_TOKEN || '';
...
const app = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST') {
      ...
      let record = sessionId ? sessions.get(String(sessionId)) : undefined;
      if (!record) {
        if (!isInitializeRequest(body)) return json(res, 400, { error: 'initialize_required' });
        const agent = new BrowserAgent({ headless: process.env.BROWSER_HEADLESS !== 'false' });
        const server = buildServer(agent);
        const transport = new StreamableHTTPServerTransport({
          sessionIdGenerator: () => crypto.randomUUID(),
          onsessioninitialized: id => sessions.set(id, { server, transport, agent })
        });
        transport.onclose = async () => {
          for (const [id, value] of sessions) if (value.transport === transport) sessions.delete(id);
          await agent.end().catch(() => {});
        };
        await server.connect(transport);
        record = { server, transport, agent };
      }
      return await record.transport.handleRequest(req, res, body);
    }

    const record = sessionId ? sessions.get(String(sessionId)) : undefined;
    if (!record) return json(res, 400, { error: 'invalid_or_missing_session' });
    if (req.method === 'GET' || req.method === 'DELETE') return await record.transport.handleRequest(req, res);
...
async function shutdown() {
  for (const { agent, transport } of sessions.values()) {
    await agent.end().catch(() => {});
...
app.listen(port, host, () => console.log(`BROWSER_CONNECTOR_HTTP_READY http://${host}:${port}/mcp`));
